...
- ZIP 导出生成项目源码。
- 可选 SiliconFlow `Pro/MiniMaxAI/MiniMax-M2.5` 模型接入，未配置时使用本地确定性生成器。
...
| ZIP 导出 | JSZip |
| 图标 | lucide-react |
| AI Provider | SiliconFlow Chat Completions 兼容接口，可选 |
...
如需接入 SiliconFlow 的 `Pro/MiniMaxAI/MiniMax-M2.5` 模型，复制 `.env.example` 为 `.env.local`：
...
`SILICONFLOW_BASE_URL` 默认使用 SiliconFlow 官方兼容接口地址，通常不需要修改。
