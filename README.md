...
- ZIP 导出生成项目源码。
- 可选 Compass `glm-5` 模型接入，未配置时使用本地确定性生成器。
...
| ZIP 导出 | JSZip |
| 图标 | lucide-react |
| AI Provider | Compass Chat Completions 兼容接口，可选 |
...
如需接入 Compass 的 `glm-5` 模型，复制 `.env.example` 为 `.env.local`：

```bash
LLM_PROVIDER=compass
COMPASS_BASE_URL=http://compass.llm.shopee.io/compass-api/v1
COMPASS_API_KEY=your_api_key
COMPASS_MODEL=glm-5
COMPASS_TIMEOUT_MS=180000
```

`COMPASS_TIMEOUT_MS` 默认是 `180000`，与 `listing-ai-v1` 的 Compass 接入时长保持一致。
如果你的部署环境允许更长函数执行时间，可以按需调大。
`COMPASS_BASE_URL` 默认对齐 `listing-ai-v1` 中的 Compass 地址，通常不需要修改。
