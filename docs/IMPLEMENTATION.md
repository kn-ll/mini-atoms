...
| 层 | 说明 |
| --- | --- |
| DeepSeek Provider | 按官方文档通过 DeepSeek 的 Chat Completions 兼容接口接入，默认 `LLM_PROVIDER=deepseek`，配置 `DEEPSEEK_API_KEY` 后启用 `deepseek-v4-flash`，并使用 `response_format={"type":"json_object"}` 保证 JSON 输出 |
